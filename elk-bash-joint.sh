...
sed -i '/network.host/c\network.host: 104.196.106.115' /etc/elasticsearch/elasticsearch.yml
...
echo 'server {
     listen 80;
     server_name 104.196.106.115;
     auth_basic "Restricted Access";
     auth_basic_user_file /etc/nginx/htpasswd.users;

    location / {
        proxy_pass http://104.1096.106.115:5601;
...
#chmod +x load.sh
#
#sed -i '/localhost/c\http://104.196.106.115:9200/' load.sh
#
#sh load.sh
#
#cd /opt
...
#
#
#curl -XPUT 'http://104.196.106.115:9200/_template/filebeat?pretty' -d@filebeat-index-template.json
#
